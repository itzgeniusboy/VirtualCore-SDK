...
public class GmsCore {
    ...
    public static void init(Context context) {
        if (sInit) return;
        sInit = true;
        // ✅ Fix: Pre-initialize WebView settings for login flows
        initLoginWebView(context);
    }
    ...
    public static Intent FacebookRedirect(Intent intent) {
        ...
        try {
            // ✅ Fix: Allow external Facebook/Browser resolution for OAuth
    ...
    public static void initLoginWebView(Context context) {
        ...
        try {
            // ✅ Fix: Set process-safe data directory suffix for WebView (Android P+)
            if (!sWebViewInit && Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                synchronized (GmsCore.class) {
                    if (!sWebViewInit) {
                        try {
                            String suffix = "bb_u" + BActivityThread.getUserId() + "_p" + android.os.Process.myPid();
            ...
            // ✅ Fix: Universal modern Chrome UA for Google/FB/Discord/Twitter
